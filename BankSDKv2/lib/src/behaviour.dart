...
class BehaviourManager {
  ...
  // 🔹 Your Flask backend URL (update with your machine IP + port)
            ...
            onPressed: () async {
              try {
                final response = await http.post(
                  ...
                  body: {"phone": phone, "otp": _otpController.text},
                );

                if (response.statusCode == 200 &&
                    response.body.contains("✅ OTP verified")) {
                  Navigator.pop(_context!);
                  _authResetCount++;
                  restoreTrust();
                  ScaffoldMessenger.of(_context!).showSnackBar(
  ...
  void _showQuestionDialog() async {
    ...
    final prefs = await SharedPreferences.getInstance();
    String savedQuestion = prefs.getString("security_question") ?? "Security Question";
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Text(savedQuestion), // ✅ dynamic question
            TextField(
